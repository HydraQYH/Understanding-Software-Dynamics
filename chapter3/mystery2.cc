...
// Make an array bigger than any expected cache size -- qiyuhang: 64 MiB
static const int kMaxArraySize = 64 * 1024 * 1024;
...
int64 NaiveTiming(uint8* ptr, int bytesize, int bytestride) {
  ...
  // qiyuhang: Max read size -- 256 * 4K = 1 MiB
...
void  FindCacheSizes(uint8* ptr, int kMaxArraySize, int linesize) {
  ...
  // Load 16 to 1M cache lines and time it. 64MB / 64B linesize = 1M lines.
  // qiyuhang: There is no Associativity Conflict!
...
void  FindCacheSizesFineGrainedL1(uint8* ptr, int kMaxArraySize, int linesize) {
  bool makelinear = false;
  const Pair* pairptr = MakeLongList(ptr, kMaxArraySize, linesize, makelinear);

  for (int lgcount = 0; lgcount <= 8; ++lgcount) {
    int count = 512 + lgcount * linesize;
 
    // Try to force the data we will access out of the caches
    TrashTheCaches(ptr, kMaxArraySize);

   // Repeat four times. First will take cache misses, others will not if small enough
    fprintf(stdout, "FineGrained for L1: count--[%d], size--[%d]K, giving cy/ld. Repeat.  ", count, count * linesize / 1024);
    for (int i = 0; i < 4; ++i) {
      int64 cyclesperload = ScrambledLoads(pairptr, count);
      fprintf(stdout, "%lld ", cyclesperload);
    }
...
void  FindCacheSizesFineGrainedL2(uint8* ptr, int kMaxArraySize, int linesize) {
  bool makelinear = false;
  const Pair* pairptr = MakeLongList(ptr, kMaxArraySize, linesize, makelinear);

  for (int lgcount = 0; lgcount <= 12; ++lgcount) {
    int count = 8192 + lgcount * 2048;
 
    // Try to force the data we will access out of the caches
    TrashTheCaches(ptr, kMaxArraySize);

   // Repeat four times. First will take cache misses, others will not if small enough
    fprintf(stdout, "FineGrained for L2: count--[%d], size--[%d]K, giving cy/ld. Repeat.  ", count, count * linesize / 1024);
    for (int i = 0; i < 4; ++i) {
      int64 cyclesperload = ScrambledLoads(pairptr, count);
      fprintf(stdout, "%lld ", cyclesperload);
    }
...
void  FindCacheAssociativity(uint8* ptr, int kMaxArraySize, int linesize, int totalsize) {
  ...
  bool makelinear = false;
  const Pair* pairptr = MakeLongList(ptr, kMaxArraySize, kPageSize, makelinear);

  for (int associativity = 4; associativity <= 32; associativity += 4) {
    // Try to force the data we will access out of the caches
    TrashTheCaches(ptr, kMaxArraySize);

    fprintf(stdout, "Associativity %d: ", associativity);
    int64 cyclesperload = ScrambledLoads(pairptr, associativity);
    for (int i = 0; i < 2; ++i) {
      cyclesperload = ScrambledLoads(pairptr, associativity);
      fprintf(stdout, "%lld ", cyclesperload);
    }
    fprintf(stdout, "\n");
  }
...
int main (int argc, const char** argv) {
  ...
  FindCacheSizesFineGrainedL1(ptr, kMaxArraySize, linesize);
  FindCacheSizesFineGrainedL2(ptr, kMaxArraySize, linesize);

  // We will learn that the cache sizes for L1 and L2 are 48KB and 1280KB respectively,
  // and for L3 either 4MB (dclab-1) or 3MB (dclab-2)  

  int l1_total_size = 48 * 1024;
  FindCacheAssociativity(ptr, kMaxArraySize, linesize, l1_total_size);

  int l2_total_size = 1280 * 1024;
  // FindCacheAssociativity(ptr, kMaxArraySize, linesize, l2_total_size);

  int l3_total_size = 54 * 1024 * 1024;
